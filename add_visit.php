...
if (
	...
	if ((int) $_POST["VisitStatus"] === 1) {
		// Expected visit only - do not save check-in or checkout time
		$_POST["CheckinTime"] = "_NULL_";
		$_POST["CheckoutTime"] = "_NULL_";
	}

	if ((int) $_POST["VisitStatus"] !== 3) {
		$_POST["CheckoutTime"] = "_NULL_";
	}
...
<script>
	jQuery().ready(function ($) {
		...
		$('#VisitorName').on('input blur', function () {
			...
			if (value !== "") {
				$(this).val(value.replace(/\b\w/g, function (letter) {
					return letter.toUpperCase();
				}));
			}

			// Trim only for validation, not while typing. This allows space after first name.
		...
		var currentCompany = $("#CompanyName").val();

		if (currentCompany !== "") {
			var companies = ["Mobis", "Transys", "HMGMA", "Glovis", "Hyundai Steel", "QS Contractor"];

			if ($.inArray(currentCompany, companies) >= 0) {
				$("#CompanySelect").show().val(currentCompany).trigger("change");
			} else {
				$("#CompanySelect").hide().val("CUSTOM");
				$("#CompanyName")
					.show()
					.val(currentCompany)
					.prop("readonly", false);
		...
		$("#submitdatav").click(function () {
			...
			if ($("#CompanyName").is(":visible") && $.trim($("#HostID").val()) == "") { err = 1; $('#HostID').css({ "border-color": "red", "background-color": "#F79BB7" }); } else { $('#HostID').css({ "border-color": "#bdbdbd", "background-color": "#FFF" }); }
					...
					<div class="form-group">
						...
						<div class="col-md-6">
							<input type="text" class="form-control" id="VisitorName" name="VisitorName"
								placeholder="First/Middle name Last name" autocomplete="off"
					...
					<div class="form-group">
						...
						<div class="col-md-6">
							...
							<select id="CompanySelect" name="CompanySelect" class="form-control">
								...
								<option value="CUSTOM">Type Company</option>
							</select>

							<input type="text" class="form-control" id="CompanyName" name="CompanyName"
								style="display:none; margin-top:8px;" placeholder="Enter Company Name"
								autocomplete="off"
			...
			<div class="row">
				...
				<div class="col-sm-6">
					<div class="form-group">
						...
						<div class="col-md-2">
							<input type="text" class="form-control" id="BadgeID" name="BadgeID" maxlength="8"
								value="<?php echo (empty($vData[0]["BadgeID"])) ? "" : $vData[0]["BadgeID"]; ?>">
						</div>
					</div>
				</div>
			...
			<div class="row">
				<div class="col-sm-6">
					<div class="form-group">
						...
						<div class="col-md-6">
							<textarea class="form-control frmss" id="VisitAbout" name="VisitAbout"
								autocomplete="off"><?php echo (empty($vData[0]["VisitAbout"])) ? "" : $vData[0]["VisitAbout"]; ?></textarea>
