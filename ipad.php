...
if (!empty($_POST["FirstName"]) || !empty($_POST["LastName"]) || !empty($_POST["VisitorName"])) {
	...
	*/ // removed since 11/12/19
	...
	if ($_POST["EmpID"] !== '') {
		...
		$companySql = (trim((string) ($_POST["CompanyName"] ?? "")) !== "")
			? " AND CompanyName = " . $pdo->quote(trim((string) $_POST["CompanyName"]))
			: "";

		$empVisitor = $pdo->query(
			"SELECT VisitorID, FirstName, LastName
         FROM Visitors
         WHERE EmpID = " . $pdo->quote($_POST["EmpID"]) . "
         " . $companySql . "
         LIMIT 1"
		)->fetch(PDO::FETCH_ASSOC) ?: null;
	}
	...
	if (empty($_POST["VisitorID"])) { // new visitor , insert		
		...
		if (empty($_POST["argtsign"]) && in_array($center, $centerWA)) { // KGTC agreement/signature only
			...
			if ($pdo->query($qryup))
				writeToLog($pdo, 18, " - Agreement signed by " . $_POST["VisitorName"] . " - Email: " . $_POST["Email"]);
		}
		...
		jQuery().ready(function ($) {
			...
			$("#CompanySelect").change(function () {
				...
				if (company === "") {
					...
					$("#VisitAbout").val("");
					$("#HostID").val("").prop("disabled", false);
					$("#hostLabel").html("Host");
					return;
				}
			...
			$("#checkin").click(function () {
				...
				if (err == 0 && em == 0) {
					var company = $("#CompanyName").is(":visible")
						? $("#CompanyName").val()
						: $("#CompanySelect").val();
...
<body>
			...
			<div class="panel panel-sign">
				...
				<div class="panel-body">
					<div class="alert alert-info">
						<p class="m-none text-semibold h6">If you are expected today, type 2 or more characters of your
							first or last name and select your row from the list to display your information.</p>
					</div>
					<form name="fvisit" id="fvisit" method="post" action="ipad.php">
						...
						<input type="hidden" name="argtsign" id="argtsign" value="" /><input type="hidden"
							name="VisitID" id="VisitID" value="" />
							...
							<div class="row">
								<div class="col-sm-6 mb-lg">
									...
									<input name="CompanyName" id="CompanyName" type="text" class="form-control"
										style="display:none;" placeholder="Enter Company Name" autocomplete="off">
								</div>
								<div class="col-sm-6 mb-lg">
									...
									<div id="ipad-host-options-all" class="hidden" aria-hidden="true">
										<?php echo $ipadHostOptionsAll; ?>
									</div>
									<div id="ipad-host-options-center" class="hidden" aria-hidden="true">
										<?php echo $ipadHostOptionsCenter; ?>
									</div>
								</div>
							</div>
						</div>
						...
						<input name="VisitAbout" id="VisitAbout" type="text" class="form-control"
							placeholder="Visit reason / purpose" autocomplete="off" />

						<!-- signature removed since 11/12/19 -- only KGTC since 12/4/19 -->
						<?php if (in_array($center, $centerWA)) { ?>
							<div class="row">
								<div class="col-sm-12" id="chk_ter">
									<label for="AgreeTerms">Click <a href="#modal-text" class="call-modal"
											title="Clicking this link shows the terms of agreement">here</a> to read and
										sign the terms of agreement</a> <span class="required">*</span></label>
								</div>
							</div>
						<?php } ?>
						<div class="row">
							...
							<div class="col-sm-4">
								<input type="button" class="btn btn-dark btn-block btn-lg mt-lg" id="resetvisitor"
									value="Reset">
							</div>
							<div class="col-sm-4 text-right">
								<input type="button" class="btn btn-chkin btn-block btn-lg mt-lg" id="checkin"
									value="Check In">
							</div>
						</div>
	...
	<?php if (in_array($center, $centerWA))
		include("modalterms{$center}.php"); /*removed since 11/12/19 -- only KGTC since 12/4/19  */ ?>
	<!-- end: page -->
