...
<section class="panel">
	<header class="panel-heading">
		...
		?>
		<h2 class="panel-title"><?php echo strtoupper($_SESSION["center"]); ?> - Total Number of visits for <span
				id="vtxtDate"
				class="text-tertiary"><?php echo (!empty($_POST["daty"])) ? $_POST["daty"] : date("m/d/Y"); ?></span> :
			<span id="num_visit"><?php echo $rscount[0]; ?></span>
		</h2>
	</header>
		...
		<div class="panel-body">
			<?php if ($daty == date("Y-m-d")) { ?>
				<span class="text-success"
					id="msg_update"><?php echo isset($msgact) ? $msgact[$_GET["act"] - 1] : ''; ?></span><br>
				Select table row(s) below, then click on the following button to: <br>
				<input type="button" id="btncheckin" value="Check In" class="btn btn-success m-xs" data-toggle="tooltip"
					data-placement="top" title="Check in selected Visit(s)" />&nbsp; &nbsp;
				<input type="button" id="btncheckout" value="Check Out" class="btn btn-info m-xs" data-toggle="tooltip"
					data-placement="top" title="Check out selected Visit(s)" />&nbsp; &nbsp;
				...
				<input type="button" id="btntablesup" value="Delete selected Visit(s)" class="btn btn-warning m-xs"
					data-toggle="tooltip" data-placement="top" title="Delete selected Visit(s)" />&nbsp; &nbsp;
				<input type="button" id="btntableunsel" value="Unselect Selected rows" class="btn m-xs" data-toggle="tooltip"
					data-placement="top" title="Unselect Selected rows" />&nbsp; &nbsp;
				<input type="button" id="btntableselall" value="Select all rows" class="btn btn-dark m-xs" data-toggle="tooltip"
					data-placement="top" title="Select all rows" /><br /><br>
			<?php } ?>
				...
				<tbody>
					...
					foreach ($pdo->query($qrylist) as $rowlist) {
						...
						<tr class="gradeX">
							...
							<td id="v_<?php echo $rowlist["VisitID"]; ?>"
								class="text-semibold <?php echo $arCssStatus[$rowlist["visitStatusNum"] - 1]; ?>">
								<?php echo $rowlist["VisitStatus"] . $sysico; ?>
							</td>
							<td><?php echo $rowlist["vname"]; ?></td>
							<td class="center hidden-phone" style="width:80px;white-space:nowrap;">
								<?php echo $rowlist["CheckinTime2"]; ?>
							</td>

							<td id="t_<?php echo $rowlist["VisitID"]; ?>" class="center hidden-phone"
								style="width:80px;white-space:nowrap;"><?php echo $rowlist["CheckoutTime"]; ?></td>
							...
							<td class="center" style="width:40px;">
								<a href="admin.php?p=add_visit&vid=<?php echo $rowlist["VisitID"]; ?>"
									class="on-default edit-row"><i class="fa fa-pencil"></i></a>
							</td>
							<td class="center" style="width:40px;">
								<a href="#" class="on-default remove-row"><i class="text-warning fa fa-trash-o"></i><input
										type="hidden" id="id_<?php echo $rowlist["VisitID"]; ?>"
										value="%%<?php echo $rowlist["VisitID"]; ?>%%"></a>
							</td>
						...
						success: function (msg) {
							...
							for (var i = 0; i < nbsel; i++) {
								if (rows[i][1] == "Checked in") {
									$("#t_" + rows[i][10]).html(currentTime);
									$("#v_" + rows[i][10]).html(css_status);
									rows[i][1] = status;
								}
							}
